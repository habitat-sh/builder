    ...
    it('allows me to search for packages', function (done) {
      request.get('/depot/pkgs/search/testapp')
        ...
        .end(function (err, res) {
          ...
          expect(res.body.data[1].release).to.equal(release7);
          ...
          expect(res.body.data[2].release).to.equal(release4);
          expect(res.body.data[5].origin).to.equal('neurosis');
          expect(res.body.data[5].name).to.equal('testapp');
          expect(res.body.data[5].version).to.equal('0.1.3');
          expect(res.body.data[5].release).to.equal(release1);
          done(err);
        });
    });
