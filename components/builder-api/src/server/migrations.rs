use crate::{
    db::models::keys::OriginPrivateSigningKey,
    server::error::{Error, Result},
};
use builder_core::{crypto, keys};
use diesel::pg::PgConnection;
use habitat_core::crypto::keys::{Key, KeyCache};
...
pub fn migrate_to_encrypted(conn: &mut PgConnection, key_cache: &KeyCache) -> Result<()> {
    ...
    loop {
        let skeys = OriginPrivateSigningKey::list_unencrypted(
            next_id,
        ...
        warn!(
            "migrate_to_encrypted found {}/{} keys requested",
            skeys.len(),
            KEY_MIGRATION_CHUNK_SIZE
        );
        if skeys.is_empty() {
        ...
        for skey in skeys {
            ...
            if skey.encryption_key_rev.is_none() {
                ...
                OriginPrivateSigningKey::update_key(
                    skey.id,
                    &encrypted_key,
    ...
    warn!(
        "migrate_to_encrypted complete in {} sec, updated {}, skipped {} as already updated",
