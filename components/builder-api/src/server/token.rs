...
use habitat_builder_db::models::{account::{Account, AccountToken, NewAccount, NewAccountToken}, channel::{Channel, CreateChannel}, origin::{NewOrigin, Origin}, package::PackageVisibility};
...
pub fn provision_bldr_environment(app_state: &AppState) -> Result<String, Error> {
    ...
    for origin in &app_state.config.provision.origins {
        ...
        match Origin::create(&new_origin, &conn) {
            ...
            Err(DieselError::DatabaseError(DatabaseErrorKind::UniqueViolation, _)) => {
                // If there is a unique violation error (conflict), log that the origin already exists.
    ...
    let filtered_channels: Vec<String> = app_state.config.provision.channels
        .iter()
    ...
    for (origin, channel) in app_state.config.provision.origins.iter().zip(&filtered_channels) {
        ...
        match Channel::create(&new_channel, &conn) {
            ...
            Err(DieselError::DatabaseError(DatabaseErrorKind::UniqueViolation, _)) => {
                // If there is a unique violation error (conflict), log that the origin already exists.
                debug!("Channel {} for Origin {} already exists, skipping creation.", channel, origin);
            }
            Err(err) => {
                error!("Failed to create channel {} for origin {}, err={:?}", channel, origin, err); // Log the error
                return Err(Error::DieselError(err));
            }
        }
    }
    ...
    let token_file_path = app_state.config.provision.token_path.join(BLDR_TOKEN_FILE_NAME);
