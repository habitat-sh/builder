...
use crate::{
    ...
    models::package::{BuilderPackageIdent, PackageVisibility, PackageWithVersionArray},
    schema::{
        audit::{audit_package, audit_package_group},
        channel::{origin_channel_packages, origin_channels},
        member::origin_members,
        origin::origins,
        package::{origin_packages, origin_packages_with_version_array},
    },
};
...
use diesel::{
    self,
    dsl::{count, count_star, sql, IntervalDsl},
    pg::PgConnection,
    prelude::*,
    result::QueryResult,
...
impl Channel {
    ...
    pub fn channel_for_group(group_id: u64) -> String {
        format!("bldr-{}", group_id)
    }
    ...
    pub fn list_latest_packages(
        req: &ListAllChannelPackagesForTarget,
        conn: &mut PgConnection,
        ...
        let duration_millis = start_time.elapsed().as_millis();
        trace!(
            "DBCall channel::list_latest_package time: {} ms",
            duration_millis
        );
        Histogram::DbCallTime.set(duration_millis as f64);
    ...
    pub fn list_packages(
        ...
        count_query = count_query
            .filter(origin_packages::ident_array.contains(ident_parts.clone()))
        ...
        let idents: Vec<String> = page_base
        ...
        let pkgs: Vec<BuilderPackageIdent> = idents
            .into_iter()
            .map(|ident| {
    ...
    pub fn list_all_packages(
        ...
        let duration_millis = start_time.elapsed().as_millis();
        trace!(
            "DBCall channel::list_all_packages time: {} ms",
            duration_millis
        );
        Histogram::DbCallTime.set(duration_millis as f64);
    ...
    pub fn list_all_packages_by_channel_id(
        channel_id: i64,
        ...
        // TODO check that this join is using an appropriate index
        let result = origin_packages::table
            .inner_join(origin_channel_packages::table)
        ...
        let duration_millis = start_time.elapsed().as_millis();
        trace!(
            "DBCall channel::list_all_packages_by_channel_id time: {} ms",
            duration_millis
        );
        Histogram::DbCallTime.set(duration_millis as f64);
        Histogram::ChannelListAllPackagesCallTime.set(duration_millis as f64);
    ...
    pub fn count_origin_channels(origin: &str, conn: &mut PgConnection) -> QueryResult<i64> {
        Counter::DBCall.increment();
        origin_channels::table
            .select(count(origin_channels::id))
            .filter(origin_channels::origin.eq(&origin))
            .first(conn)
    }
    ...
    //
    pub fn do_promote_or_demote_packages_cross_channels(
        ch_source: i64,
        ch_target: i64,
        promote: bool,
        conn: &mut PgConnection,
...
impl AuditPackage {
    pub fn list(el: &ListEvents, conn: &mut PgConnection) -> QueryResult<(Vec<AuditPackage>, i64)> {
        ...
        if let Some(session_id) = el.account_id {
            let origins = origin_members::table
        ...
        let events: Vec<AuditPackage> = page_query
            .order((
                audit_package::created_at.desc(),
