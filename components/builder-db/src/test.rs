...
macro_rules! datastore_test {
    ($datastore:ident) => {{
        ...
        INIT_TEMPLATE.call_once(|| {
            ...
            diesel::sql_query(format!("CREATE DATABASE {}", db_template).as_str()).execute(&conn)
            ...
            let _ = habitat_builder_db::migration::setup(&diesel_pool.get_conn().unwrap());

            // Run jobsrv migrations
            let ds = $datastore::from_pool(diesel_pool.clone());
