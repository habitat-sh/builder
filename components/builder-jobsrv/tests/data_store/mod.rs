...
fn next_pending_job() {
    ...
    // and worker set to the passed in worker id
    let pending_job_2 = ds
        .next_pending_job("worker2")
        .expect("Failed to get pending job");
...
fn get_job_group() {
    ...
    let ds = datastore_test!(DataStore);
    let group1 = ds
        .create_job_group(&msg, project_names.clone())
        .expect("Failed to create a group");
    let group2 = ds
        .create_job_group(&msg, project_names.clone())
        .expect("Failed to create a group");
    ...
    let g1 = ds
        .get_job_group(&get_msg1)
        .expect("Failed to get group 1")
        .expect("Group should exist");
    let g2 = ds
        .get_job_group(&get_msg2)
        .expect("Failed to get group 2")
        .expect("Group should exist");
    let g3 = ds
        .get_job_group(&get_msg3)
...
fn pending_groups() {
    ...
    let queued_groups = ds
        .get_queued_job_groups()
    ...
    let pending_groups_0 = ds
        .pending_job_groups(1)
        .expect("Failed to get pendings group");
    assert_eq!(pending_groups_0.len(), 0, "Found unexpected pending group");

    let group2 = ds
        .create_job_group(&msg2, project_names.clone())
        .expect("Failed to create a group");

    let group3 = ds
        .create_job_group(&msg3, project_names.clone())
        .expect("Failed to create a group");
    ...
    let remaining_groups = ds
        .pending_job_groups(5)
...
fn set_job_group_state() {
    ...
    let pending_group = ds
        .get_job_group(&get_msg)
...
fn get_graph_stats() {
    ...
    let stats = ds
        .get_job_graph_package_stats(&stats_msg)
