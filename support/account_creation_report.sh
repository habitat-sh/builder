...
if command -v hab > /dev/null 2>&1; then
  if hab pkg exec core/postgresql psql -U hab -q -c "COPY (SELECT * FROM account_creation_report_v2('$1')) TO '$output' WITH (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *)" builder_sessionsrv; then
